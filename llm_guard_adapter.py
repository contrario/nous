"""LLM Guard scan_output evidence adapter (DARK, opt-in).

Consume-only integration. Parses an LLM Guard scan_output result (normalized to
a dict with results_valid, results_score, sanitized_output, prompt,
original_output) by KEY (never imports the llm_guard package) and emits a NOUS
evidence dossier over it: a signed, drop-when-None adapter manifest binding two
digests (upstream + projection), with an optional Rekor v2 anchor of the
canonical projected payload. Mirrors the shipped santander_adapter.py structure;
the only per-adapter differences are the projection map, the source_kind, the
producer tag, and the ABSENCE of an entropy-tie leg (a scan result carries no
nonce, so the Rekor leaf digest == projection_digest directly).

Honest boundary (inviolable):
  - EVIDENCES, never proves. The dossier evidences that a scan_output result
    with these per-scanner verdicts was produced. It does NOT prove the scanners
    sound, the sanitization complete, or the output safe. "proves" is reserved
    for Z3/Farkas.
  - MONITOR, not guard. Consumes the result after the fact; enforces nothing on
    LLM Guard; blocks no output.
  - HASHES, not raw. sanitized_output, prompt, and original_output can carry
    model output or user PII; they are carried as sha256 only. Raw values are
    the auditor's out-of-band pack.
...
cryptography and no new claim class. Writes nothing unless NOUS_LLM_GUARD_ADAPTER
is set. VALUE-ADD is real ONLY because an LLM Guard scan result is UNSIGNED.
"""
# __s219_llm_guard_adapter_v1__
...
# Consume-only projection expects a dict with per-scanner results_valid and
# results_score. The left set is the totality arbiter; a record missing any
# expected key is a typed refusal (axiom 5).
...
class LLMGuardAdapterError(RuntimeError):
...
def _require(record: Mapping[str, Any], key: str) -> Any:
    if key not in record:
        raise LLMGuardAdapterError(
            "missing expected key: " + repr(key) + " (refuse over guess)"
        )
    return record[key]


def project_llmguard(record: Mapping[str, Any]) -> dict[str, Any]:
    """Total map scan_output result -> canonical decision-structural payload.
    Overall decision + per-scanner verdicts/scores carried in clear; free-text
    fields carried as sha256 only; drop-when-None so unrelated dossiers stay
    byte-identical. Contains NO raw model output, prompt, or sanitized plaintext."""
    for key in _EXPECTED_KEYS:
        _require(record, key)
    valid = record["results_valid"]
    score = record["results_score"]
    if not isinstance(valid, dict) or not isinstance(score, dict):
        raise LLMGuardAdapterError(
            "results_valid and results_score must be dicts"
        )
    projection: dict[str, Any] = {
        "schema": "nous/llm-guard-scan/v1",
        "projection_version": LLM_GUARD_PROJECTION_VERSION,
        "decision": (
            "pass" if all(bool(x) for x in valid.values()) else "block"
        ),
        "scanners": [
            {
                "name": n,
        ...
        "sanitized_output_sha256": _sha256_hex(record.get("sanitized_output")),
        "prompt_sha256": _sha256_hex(record.get("prompt")),
        "original_output_sha256": _sha256_hex(record.get("original_output")),
    }
...
class LLMGuardDossierManifest:
    ...
    No entropy leg: a scan result carries no nonce."""
    nous_version: str
    upstream_digest: str
    projection_digest: str
    timestamp_utc: str
    schema_version: int = LLM_GUARD_ADAPTER_SCHEMA_VERSION
    source_kind: str = LLM_GUARD_SOURCE_KIND
    projection_version: str = LLM_GUARD_PROJECTION_VERSION
...
def _public_body(manifest: LLMGuardDossierManifest) -> bytes:
    doc = {
...
def dossier_commitment(manifest: LLMGuardDossierManifest) -> bytes:
    ...
    return hashlib.sha256(
        LLM_GUARD_COMMIT_TAG + _public_body(manifest)
    ).digest()
...
def build_dossier(
    ...
    """Build the full signed dossier document over a scan_output result.
    ...
    payload = project_llmguard(record)
    manifest = LLMGuardDossierManifest(
...
def verify_llmguard_dossier(
    ...
    *,
    ...
    kind_ok = manifest_doc.get("source_kind") == LLM_GUARD_SOURCE_KIND
...
"""Offline verification of a NOUS LLM Guard scan dossier.
...
Checks, fail-closed, in order:
  ...
  2. source_kind == "llm-guard/scan/decision".
...
BOUNDARY: a verified dossier EVIDENCES that this scan_output result was
produced, and (when anchored) that the exact projected payload is publicly
logged and log-ordered. It does NOT establish a trusted time. It does NOT prove
the scanners sound or the output safe. evidences-not-proves.
"""
import base64
import hashlib
import json
...
SOURCE_KIND = "llm-guard/scan/decision"
...
def main():
    ...
    print("result: scan-evidenced")
    print("boundary: evidences this scan_output result was produced"
          ...
          + "; NOT that the scanners are sound or the output safe; "
...
def emit_dossier_to_dir(
    ...
    """DARK write path. Refuses unless NOUS_LLM_GUARD_ADAPTER is set; writes
