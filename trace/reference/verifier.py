...
# copy below: edit rekor_check.py and re-splice, or the drift test
# test_splice_matches_the_tracked_source will fail. The reference verifier is
# standalone (stdlib + cryptography, no sibling imports), so the leg cannot be
# imported and must travel as bytes.
# __rk_splice_begin_v1__
_RK_SIG_PREFIX = "\u2014 "
...
def verify_pack(pack):
    ...
    for c in ckpts:
        ...
        if atype == "rfc3161-sim":
            ...
            # __rk_dispatch_v1__ SPEC 10.1 transparency-log backend. Rekor v2
            # carries no per-entry integrated time and no SET, so trusted time
            # can only come from an RFC 3161 token over the leaf signature.
            # Inclusion WITH time and inclusion WITHOUT it are different claim
            # classes and are reported as different states -- never one
            # verdict with a note.
            # __rk_logkeys_rename_v1__ NEVER bind `keys` here: verify_pack
            # binds it to the keys manifest and the SPEC 10.3 loop below
            # depends on it. Shadowing it turned a valid pack into a KeyError.
