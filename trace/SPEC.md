...
**Version:** 0.2.4-draft
**Status:** Implementation-validated. Supersedes 0.2.3-draft.
...
- **`both`**: reserved. The composite backend is not implemented in this revision, and a Pack declaring it is refused as an unverifiable anchor type (§10.1, fail-closed). No RFC 2119 recommendation is made for a backend that no conforming Verifier can verify; the intended property is transparency and trusted time together, with the transparency leg evidencing membership and the timestamp leg bound directly to the signed root.
