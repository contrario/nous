...
**Version:** 0.2.7-draft
**Status:** Implementation-validated. Supersedes 0.2.6-draft.
...
**Changes from 0.2.6:** §12.4 -- `JCS_TYPE` is registered. The reference Verifier has raised it since the commit that first landed that Verifier, and no revision required its registration, so the registry did not list a code the Verifier can report. §12.4 also gains an allocation rule, which no revision had stated: the registry is the complete set for the reference Verifier, a code introduced there is registered in the same change, and every other implementation, including those distributed with this project, may extend only under a private-use prefix. The defect corrected is the absence of a rule rather than a breach of one; before this revision the question of an unregistered code had no answer, so the reference Verifier was neither conforming nor non-conforming on the point. No wire change: no field, tag, hash input or encoding moved and `spec_version` remains `"0.2.0"`. No claim in §3 changed: that section states nothing about which reason codes a Verifier may report.
...
Fail-closed ordering is normative: the reported reason is the FIRST failing check in §12.2 order. Registry (v0.2.7; `JCS_TYPE` added in 0.2.7, `RUN_START_ANCHORING` added in 0.2.4, otherwise unchanged since v0.2.1): `SPEC_VERSION, TOLERANCE_INVALID, MANIFEST_FILE_HASH, KEYS_MANIFEST_SIG, KEY_ID_MISMATCH, OBLIGATION_MANIFEST_SIG, OBLIGATION_ID_MISMATCH, PROOF_ARTIFACT_MISSING, PROOF_ARTIFACT_HASH, ASSURANCE_INVALID, FLOAT_IN_SIGNED, INT_RANGE, JCS_TYPE, EMPTY_TRACE, TRACE_ID_MIXED, SEQ_ORDER, HASH_CHAIN_BREAK, KEY_UNKNOWN, SIG_INVALID, STRUCT_NO_RUN_START, RUN_START_KEYS_HASH, RUN_START_OBL_HASH, RUN_START_TOLERANCE, RUN_START_ANCHORING, CKPT_RANGE, MERKLE_MISMATCH, CKPT_ROOT_SIG, ANCHOR_TYPE, ANCHOR_INVALID, TIME_BOUND_VIOLATION, KEY_EXPIRED, PAYLOAD_CLASS, PAYLOAD_HASH_MISMATCH, ASSIGNMENT_MISSING, ASSIGNMENT_REF_MISSING, ASSIGNMENT_PARSE, ASSIGNMENT_VARS_MISMATCH, OBLIGATION_UNKNOWN, OBLIGATION_REF_REQUIRED, OBLIGATION_REF_FORBIDDEN, VERDICT_MISMATCH, SALT_REUSE`. Wrong-tag signatures (E4) are indistinguishable from bad signatures by construction and are reported as `SIG_INVALID`.

*Allocation (normative):* the registry above is the complete set of reason codes the reference Verifier may report. A reference Verifier MUST NOT report a reason code this section does not list, and a reason code introduced into the reference Verifier MUST be registered here in the same change. Implementations other than the reference Verifier, including those distributed with this project, MAY report additional reason codes; such codes MUST carry the prefix `X_`, are private use, and MUST NOT be assigned by this specification.
