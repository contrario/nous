...
def build_dossier(
    ...
    *,
    ...
    _trace_bundle_src_dir = None  # __trace_bundle_carry_init_v1__
    if parsed_manifest.trace_bundle_sha256 is not None:  # __trace_bundle_carry_read_v1__
        import hashlib as _tb_hashlib
        trace_bundle_src = manifest.parent / "trace_bundle"
        _tb_manifest_src = trace_bundle_src / "manifest.json"
        if not _tb_manifest_src.is_file():
            raise DossierError(
                "manifest declares a NOUS-TRACE runtime evidence bundle "
                "but trace_bundle/manifest.json not found next to "
                "manifest: " + str(_tb_manifest_src)
            )
        _tb_manifest_bytes = _tb_manifest_src.read_bytes()
        _tb_file_sha = _tb_hashlib.sha256(_tb_manifest_bytes).hexdigest()
        if _tb_file_sha != parsed_manifest.trace_bundle_sha256:
            raise DossierError(
                "trace_bundle/manifest.json sha256 mismatch: file="
                + _tb_file_sha[:16] + "... manifest="
                + (parsed_manifest.trace_bundle_sha256 or "")[:16]
                + "... (evidence bundle trust root tampered or "
                "substituted)"
            )
        _trace_bundle_src_dir = trace_bundle_src
    _trace_bundle_anchor_bytes = None  # __tba_carry_init_v1__
    if parsed_manifest.trace_bundle_anchor_sha256 is not None:  # __tba_carry_read_v1__
        import hashlib as _tba_hashlib
        if parsed_manifest.trace_bundle_sha256 is None:
            raise DossierError(
                "manifest declares trace_bundle_anchor_sha256 but no "
                "trace_bundle_sha256; a temporal-existence receipt cannot be "
                "carried without the bundle identity it attests (C2 "
                "presupposes C1)"
            )
        _tba_src = manifest.parent / "trace_bundle.anchor.json"
        if not _tba_src.is_file():
            raise DossierError(
                "manifest declares a trace-bundle temporal anchor but "
                "trace_bundle.anchor.json not found next to manifest: "
                + str(_tba_src)
            )
        _tba_bytes = _tba_src.read_bytes()
        _tba_file_sha = _tba_hashlib.sha256(_tba_bytes).hexdigest()
        if _tba_file_sha != parsed_manifest.trace_bundle_anchor_sha256:
            raise DossierError(
                "trace_bundle.anchor.json sha256 mismatch: file="
                + _tba_file_sha[:16] + "... manifest="
                + (parsed_manifest.trace_bundle_anchor_sha256 or "")[:16]
                + "... (temporal receipt tampered or substituted)"
            )
        _trace_bundle_anchor_bytes = _tba_bytes
    ...
    if _trace_bundle_src_dir is not None:  # __trace_bundle_carry_write_v1__
        import shutil as _tb_shutil
        _tb_out = output / "trace_bundle"
        _tb_shutil.copytree(_trace_bundle_src_dir, _tb_out)
        for _tb_p in sorted(_tb_out.rglob("*")):
            if _tb_p.is_file():
                files.append(_tb_p.relative_to(output).as_posix())
    ...
    if _trace_bundle_anchor_bytes is not None:  # __tba_carry_write_v1__
        (output / "trace_bundle.anchor.json").write_bytes(
            _trace_bundle_anchor_bytes
        )
        files.append("trace_bundle.anchor.json")
    ...
    if parsed_manifest.trace_bundle_sha256 is not None:  # __trace_bundle_splice_v1__
        verify_path.write_text(
            _splice_trace_bundle_check(
                verify_path.read_text(encoding="utf-8")
            ),
            encoding="utf-8",
        )
    if parsed_manifest.trace_bundle_anchor_sha256 is not None:  # __tba_splice_v1__
        verify_path.write_text(
            _splice_trace_bundle_anchor_check(
                verify_path.read_text(encoding="utf-8")
            ),
            encoding="utf-8",
        )
