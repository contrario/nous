...
class NousTransformer(Transformer):
    ...
    def sense_call(self, items: list) -> Any:
        ...
        for idx, it in enumerate(items):
            if isinstance(it, Token) and str(it) in ("sense", "αίσθηση"):
                sense_idx = idx
                break

        if sense_idx >= 0:
            bind_name = str(items[0]) if sense_idx > 0 else None
            after = [i for i in items[sense_idx + 1:] if not isinstance(i, Token) or str(i) not in ("sense", "αίσθηση")]
            tool_name = str(after[0]) if after else "unknown"
            args = after[1] if len(after) > 1 and isinstance(after[1], dict) else {}
            if bind_name:
                return LetNode(name=bind_name, value={"kind": "sense_call", "tool": tool_name, "args": args})
            return SenseCallNode(tool_name=tool_name, args=args)

        filtered = [i for i in items if not isinstance(i, Token)]
        if len(filtered) >= 1:
            tool = str(filtered[0])
            args = filtered[1] if len(filtered) > 1 and isinstance(filtered[1], dict) else {}
    ...
    def instinct_block(self, items: list) -> InstinctNode:
        raw = [i for i in items[1:] if not isinstance(i, Token)]
        stmts = self._merge_keyword_stmts(raw)
        return InstinctNode(statements=stmts)

    def _merge_keyword_stmts(self, raw: list) -> list:
        merged: list = []
        i = 0
        while i < len(raw):
            item = raw[i]
            ...
            if isinstance(item, str) and item in ("sense", "αίσθηση"):
                if i + 1 < len(raw) and isinstance(raw[i + 1], dict) and raw[i + 1].get("kind") == "func_call":
                    fc = raw[i + 1]
                    merged.append(SenseCallNode(
                        tool_name=fc.get("func", ""),
                        args={k: v for k, v in fc.get("args", {}).items()},
                    ))
                    i += 2
                    continue
            ...
            if isinstance(item, str) and item in ("speak", "λέω", "peak"):
                if i + 1 < len(raw) and isinstance(raw[i + 1], dict):
                    ...
                    if fc.get("kind") == "func_call":
                        merged.append(SpeakNode(
                            message_type=fc.get("func", ""),
                            args={k: v for k, v in fc.get("args", {}).items() if not k.startswith("_pos_")},
                        ))
                        i += 2
                        continue
                    if fc.get("kind") == "message_construct":
                        merged.append(SpeakNode(
                            message_type=fc.get("type", ""),
                            args=fc.get("args", {}),
                        ))
                        i += 2
                        continue
            ...
            if isinstance(item, str) and item in ("guard", "φύλακας"):
                if i + 1 < len(raw):
                    cond = raw[i + 1]
                    cond = self._fix_duration_artifacts(cond)
                    merged.append(GuardNode(condition=cond))
                    i += 2
                    continue
            ...
            if isinstance(item, LetNode) and isinstance(item.value, str) and item.value in ("sense", "αίσθηση"):
                if i + 1 < len(raw) and isinstance(raw[i + 1], dict) and raw[i + 1].get("kind") == "func_call":
                    fc = raw[i + 1]
                    merged.append(LetNode(
                        name=item.name,
                        value={"kind": "sense_call", "tool": fc.get("func", ""), "args": fc.get("args", {})},
                    ))
                    i += 2
                    continue

            if isinstance(item, SenseCallNode) and item.bind_name is None:
                if i + 1 < len(raw) and isinstance(raw[i + 1], dict) and raw[i + 1].get("kind") == "func_call":
                    fc = raw[i + 1]
                    real_tool = fc.get("func", item.tool_name)
                    merged.append(SenseCallNode(
                        tool_name=real_tool,
                        args=fc.get("args", {}),
                        bind_name=item.tool_name if item.tool_name != real_tool else None,
                    ))
                    i += 2
                    continue

            if isinstance(item, GuardNode):
                item = GuardNode(condition=self._fix_duration_artifacts(item.condition))
    ...
    @staticmethod
    def _fix_duration_artifacts(expr: Any) -> Any:
        if isinstance(expr, str) and len(expr) > 1 and expr[-1] in "smhd" and expr[:-1].isdigit():
            return int(expr[:-1])
        if isinstance(expr, dict) and expr.get("kind") == "binop":
            expr = dict(expr)
            expr["left"] = NousTransformer._fix_duration_artifacts(expr["left"])
            expr["right"] = NousTransformer._fix_duration_artifacts(expr["right"])
        return expr
