...
# __s160_u2_provenance_verifier_v1__
#
# Emitter for the portable, offline NOUS SLSA-provenance verifier. Parallel to
# vsa_verifier.py: a self-contained verifier is carried as the string constant
# PROVENANCE_VERIFY_OFFLINE_PY with a single pin placeholder, and
# emit_provenance_verifier substitutes the builder public key at emit time.
#
...
__all__ = [
    "PROVENANCE_VERIFY_OFFLINE_PY",
    "BUILDER_PIN_PLACEHOLDER",
    "ProvenanceVerifierError",
    "emit_provenance_verifier",
]

BUILDER_PIN_PLACEHOLDER: str = "__NOUS_BUILDER_PINNED_PUBKEY_B64__"


class ProvenanceVerifierError(ValueError):
    """Raised when the builder public key for emission is not a valid raw
    32-byte Ed25519 key encoded as standard base64."""


PROVENANCE_VERIFY_OFFLINE_PY: str = r"""#!/usr/bin/env python3
...
_PINNED_BUILDER_PUBKEY_B64 = "__NOUS_BUILDER_PINNED_PUBKEY_B64__"
...
def _is_raw_ed25519_b64(value: str) -> bool:
    import base64
...
def emit_provenance_verifier(builder_pubkey_b64: str) -> str:
    """Return the offline provenance verifier source with the builder public
    key pinned. ``builder_pubkey_b64`` must be a raw 32-byte Ed25519 public key
    encoded as standard base64 (the same form returned by
    ``provenance.public_key_raw_b64``). Raises ProvenanceVerifierError on an
    invalid key, and refuses (same exception) if the placeholder is absent from
    or non-unique in the template."""
    if not _is_raw_ed25519_b64(builder_pubkey_b64):
        raise ProvenanceVerifierError(
            "builder_pubkey_b64 is not a valid raw 32-byte Ed25519 key in "
            "standard base64"
        )
    if PROVENANCE_VERIFY_OFFLINE_PY.count(BUILDER_PIN_PLACEHOLDER) != 1:
        raise ProvenanceVerifierError(
            "verifier template must contain exactly one builder pin placeholder"
        )
    return PROVENANCE_VERIFY_OFFLINE_PY.replace(
        BUILDER_PIN_PLACEHOLDER, builder_pubkey_b64
    )
