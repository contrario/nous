# Guardrails AI ValidationOutcome evidence adapter

A consume-only NOUS evidence adapter over a Guardrails AI `ValidationOutcome`.
It ingests the framework's own result object, projects it to a canonical
decision-structural payload, and emits a signed, offline-verifiable dossier.
It imports nothing from `guardrails`; it parses the record by key. The adapter
...
producer tag. There is no entropy leg -- a `ValidationOutcome` carries no nonce.
...
A `ValidationOutcome.to_dict()` mapping. The adapter reads the validation
decision, the per-validator results, the call identifier, and the digests of the
raw and validated model output. It refuses malformed input with a typed error.

## The projection

`project_guardrails` maps the record to a canonical payload:

- `decision`: pass or fail.
- per-validator entries `{name, passed, on_fail, error_sha256}`, sorted by name.
- `call_id`.
- `raw_output_sha256`, `validated_output_sha256`.

Free-text fields (raw model output, validated output, per-validator error
messages) are carried as sha256 commitments only, never as raw text.
...
The producer-side envelope-ledger domain tag is
`nous/guardrails-validation/v1|`. It was FROZEN at S221 by a local genesis
persistent-key signature over `dossier_commitment(genesis_manifest) =
sha256(tag || public_body)`; the dedicated key binds these exact tag bytes, so a
later tag edit invalidates the frozen signature. The dedicated key is
`guardrails-adapter.key` (Ed25519, mode 0600, under the XDG keys directory),
byte-identical in discipline to `santander-adapter.key`. The tag is not part of
the signed manifest; it participates only in the envelope commitment.
...
- No entropy leg: a `ValidationOutcome` has no nonce to anchor.
- Opt-in: the emit path writes nothing unless `NOUS_GUARDRAILS_ADAPTER` is set.
