...
def build_parser() -> "argparse.ArgumentParser":  # __s104_build_parser_v1__
    ...
    p.add_argument("--chain-coverage", metavar="MODE", default=None, choices=["full"], help="Chain coverage mode (--smt --supersedes only). 'full' carries per-link source.nous and emits blocking-net containment proofs across every hop, upgrading prior-link coverage from signature-attested to re-derived; requires --supersedes. Omitted: threshold-only chain (byte-identical to prior behavior).")  # __s127_chain_coverage_flag_v1__
