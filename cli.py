...
from cli_verify_cost import build_verify_cost_parser, cmd_verify_cost  # __s170_leg6b_verify_cost_v1__
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    # __s147_u3b_verify_ndec_detect_v1__
    if str(getattr(args, "file", "")).endswith(".ndec"):
        import ndec as _ndec
        return _ndec.verify_ndec_file(args.file)
    source = Path(args.file)
    if not source.exists():
        print(f"Error: file not found: {source}", file=sys.stderr)
        return 1
...
def cmd_consciousness(args: Any) -> None:
    """Show consciousness/self-reflection analysis."""
    from parser import parse_nous
    from validator import NousValidator
    ...
    print("")
    con_souls = [s for s in program.souls if s.consciousness is not None]
    if not con_souls:
        print("  No souls with consciousness found.")
        return
    for soul in con_souls:
        cn = soul.consciousness
        print(f"  \u2500\u2500 {soul.name} \u2500\u2500")
        print(f"  Goals:          {', '.join(cn.goals)}")
        print(f"  Reflect every:  {cn.reflect_every} cycles")
        print(f"  Self-model:     {cn.self_model}")
        print(f"  Goal threshold: {cn.goal_threshold}")
        print(f"  Depth:          {cn.introspection_depth}")
        print("")
    cn_proofs = [p for p in ver_result.proven if "VCS" in str(getattr(p, 'code', ''))]
    cn_warns = [w for w in ver_result.warnings if "VCS" in str(getattr(w, 'code', ''))]
    for p in cn_proofs:
...
def build_parser() -> "argparse.ArgumentParser":  # __s104_build_parser_v1__
    ...
    p.add_argument("--supersedes", metavar="PATH", default=None, help="Predecessor signed manifest this run supersedes (envelope-binding; --smt only). Verifies the predecessor Ed25519 signature and binds its canonical-body sha256 as prior_digest. REFUSED on invalid predecessor signature or no-op re-binding. The offline verifier is the actual guard; this is admission control.")  # __s119_supersedes_flag_v1__
    p.add_argument("--chain-coverage", metavar="MODE", default=None, choices=["full"], help="Chain coverage mode (--smt --supersedes only). 'full' carries per-link source.nous and re-derives each prior link's blocking net from its carried source, proving net containment across every hop (prior-link coverage completeness remains signature-attested); requires --supersedes. Omitted: threshold-only chain (byte-identical to prior behavior).")  # __s127_chain_coverage_flag_v1__  # __s128_chain_help_honesty_v1__
    p.add_argument("--materiality-against", metavar="PATH", default=None, dest="materiality_against", help="Classify this build's change vs a PRIOR .nous source (behavioral diff -> minor/material) and emit a sha-pinned materiality.json carried in the dossier. Classification, not an Article 25 proof; orthogonal to --supersedes (which binds the proof leg). Refused with --gap-witness. Omitted: no classification emitted (byte-identical).")  # __s171_materiality_arg_v1__
    p.add_argument("--materiality-threshold-pct", dest="materiality_threshold_pct", type=float, default=10.0, help="Absolute total cost delta percent at or above which --materiality-against classifies the change as material (default 10.0).")  # __s171_materiality_arg_v1__
    p.add_argument("--pce", metavar="PATH", default=None, dest="pce", help="Bind a predetermined-change envelope (EU AI Act Article 43(4) / Annex IV 2(f)) into the signed manifest. Validates well-formedness (envelope.parse_envelope), sha-pins it as pce_sha256, and writes pce.json next to the manifest; build_dossier carries it and the offline verifier evidences cumulative membership (WITHIN/OUTSIDE, monitor not gate). Requires --pce-baseline. Refused with --gap-witness. Omitted: byte-identical.")  # __s190_pce_arg_v1__
    p.add_argument("--pce-baseline", metavar="PATH", default=None, dest="pce_baseline", help="Committed baseline obligations canon the --pce envelope binds to; its sha256 must equal the envelope baseline_canon_sha256. Written as baseline.canon next to the manifest.")  # __s190_pce_arg_v1__
    p.add_argument("--pce-anchor", metavar="PATH", default=None, dest="pce_anchor", help="Bind a pre-commitment receipt (pce.anchor.json from 'nous pce-anchor') for the --pce envelope into the signed manifest as pce_anchor_sha256 and carry it as a dossier sidecar. The receipt anchored_pce_sha256 must equal the bound envelope sha256. Evidences pre-commitment-in-time; ordering vs a later change is the verifier's relational computation. Requires --pce. Omitted: byte-identical.")  # __s191_pce_anchor_arg_v1__
    p.add_argument("--witness", metavar="PATH", default=None, dest="witness", help="Bind a witnessed envelope-checkpoint sidecar (envelope.witness.json from the witness cosignature ceremony) into the signed manifest as envelope_witness_sha256 and carry it as a dossier sidecar. The offline verifier re-derives env_root from the enumerated fan and evidences k-of-n witness non-equivocation (MET/UNDER-QUORUM, monitor not gate); independence is a named trust assumption, auditor-pinned via witness_keys.json or NOUS_WITNESS_KEYS else operator-supplied (downgrade). Independent of --pce. Omitted: byte-identical.")  # __s194_envelope_witness_arg_v1__
    ...
    build_verify_cost_parser(sub)  # __s170_leg6b_verify_cost_v1__
    ...
    p = sub.add_parser("consciousness", help="Consciousness/self-reflection analysis")
    p.add_argument("file", help=".nous file to analyze")
